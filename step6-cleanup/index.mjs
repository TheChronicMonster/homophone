...
if (isAlice) {
    interact.pickWord = async () => {
        const word1 = await ask.ask(`Choose a word from the following list: ${WORD1}`, (x) => {
            ...
            if ( word1 === undefined ) {
                throw Error(`Not a valid word. Please choose from the list.`);
            }
            return word1;
        });
    ...
    interact.guessWord = async () => {
        const word2 = await ask.ask(`What is the homophone to your opponent's word? `, (x) => {
            const word2 = WORDS2[x];
            if ( word2 === undefined ) {
                throw Error(`Not a valid word, please try again.`);
            }
            return word2;
        });
...
interact.seeOutcome = async (outcome) => {
    if (OUTCOME[outcome] == undefined) {
        console.log(`The outcome is: Alice wins`)
    } else {
